...
- [ ] **Phase 2: Longitudinal Tracking & Medical Timeline**
...
- [ ] Define PostgreSQL schemas (SQLAlchemy) for `Disease`, `Medication`, `Allergy`, and `LabResult`.
- [ ] Update the AI extraction validation step to map JSON data directly to the new Knowledge Graph models.
- [ ] Build internal API services to link extracted entities to the active `Patient` and `hospital_id`.
...
- [ ] Implement AI Report Summary logic for Blood Tests & Lab Reports.
- [ ] Implement AI Report Summary logic for MRI, CT Scans, and ECGs.
- [ ] Implement AI Report Summary logic for Discharge Summaries & Prescriptions.
- [ ] Build the `AI Patient Health Summary` API endpoint (aggregating demographics, chronic diseases, current meds).
- [ ] Integrate the Health Summary endpoint into the Frontend React Dashboard.
- [ ] Build React UI components to manage and display the structured Disease History and Allergy Lists.
...
- [ ] Develop the AI Report Comparison engine to highlight changes between historical reports.
- [ ] Implement Laboratory Trend Analysis to continuously track biomarkers (e.g., Blood Sugar, HbA1c, Cholesterol).
- [ ] Build visual trend graphs in the React UI for tracked laboratory parameters.

### Sprint 4: Intelligent Medical Timeline
**Goal:** Auto-generate a chronological event feed from structured records.
- [ ] Build backend logic to merge visits, diagnoses, prescriptions, and lab findings into a unified, sorted timeline feed.
- [ ] Enhance the existing frontend Timeline UI to render these complex clinical events chronologically.
...
- [ ] Implement Prescription Intelligence to detect duplicate medications.
- [ ] Implement logic to detect drug-drug interactions.
- [ ] Implement logic to detect allergy conflicts based on patient history.
- [ ] Build Cross-Document Clinical Correlation (e.g., link medications to disease history).

### Sprint 6: AI Clinical Alerts & Case History
**Goal:** Provide real-time high-priority alerts and automated case histories.
- [ ] Build an automated alerting system for critical laboratory values and abnormal findings.
- [ ] Generate alerts for missed follow-up appointments and expiring prescriptions.
- [ ] Develop the `AI Case History Generator` endpoint to construct structured histories from raw records.
- [ ] Build the Alert UI / Notification Center in the React dashboard to display real-time warnings to doctors.
